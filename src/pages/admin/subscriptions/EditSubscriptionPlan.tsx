...
import { getSubscriptionPlan, updateSubscriptionPlan } from "@/api/admin/subscriptions";
...
const EditSubscriptionPlan = () => {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          setFormData({
            ...
            property_limit: plan.property_limit?.toString() || "",
            featured_limit: plan.featured_limit?.toString() || "",
            image_limit: plan.image_limit?.toString() || "",
            video_allowed: plan.video_allowed || false,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await updateSubscriptionPlan(Number(id), formData);
      ...
      toast.error("Subscription not updated: " + (error.message || ""));
    } finally {
