...
const MySubscriptions = () => {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      ...
      canceled: { color: 'bg-amber-100 text-amber-800', icon: Clock },
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {activeSubscription ? (
            <div className="space-y-6 mb-12">
              ...
              <Card className="bg-gradient-to-br from-emerald-50 to-green-50 border-2 border-emerald-200 rounded-2xl shadow-xl overflow-hidden">
                <CardContent className="p-0">
                  {/* Plan Header */}
                  <div className="bg-gradient-to-r from-emerald-500 to-green-500 p-8 text-white relative overflow-hidden">
                    <div className="absolute top-0 right-0 w-24 h-24 bg-white/10 rounded-full -translate-y-12 translate-x-12"></div>
                    <div className="relative z-10">
                      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
                        <div>
                          <h3 className="text-3xl font-bold">{activeSubscription?.plan?.name ?? ""}</h3>
                          <p className="text-emerald-100 text-lg">Active Subscription</p>
                        </div>
                        <div className="text-right">
                          <p className="text-4xl font-bold">${activeSubscription?.plan?.price ?? ""}</p>
                          <p className="text-emerald-100">per {activeSubscription?.plan?.duration_days ?? ""} days</p>
                        </div>
                      </div>
                  ...
                  <div className="p-8">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 pt-6 mt-6 border-t border-gray-200">
                      ...
                      <Button
                        ...
                        variant="outline"
                        className="flex-1 border-amber-300 cursor-pointer text-amber-600 hover:bg-amber-50"
                      >
            ...
            <Card className="bg-gradient-to-br from-blue-50 to-cyan-50 border-2 border-blue-200 rounded-2xl shadow-xl mb-12">
              <CardContent className="p-8 text-center">
                ...
                <Button
                  onClick={() => navigate(isAgent ? '/agent/subscription-plans' : '/subscription-plans')}
                  className="bg-gradient-to-r from-blue-600 to-emerald-600 hover:from-blue-700 hover:to-emerald-700 text-white font-bold px-8 py-3 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105"
                >
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {pastSubscriptions.map((subscription) => (
                  <Card key={subscription.id} className="bg-white border-2 border-gray-200 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300">
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          <h3 className="font-semibold text-gray-900">{subscription?.plan?.name ?? ''}</h3>
                          <p className="text-sm text-gray-500">${subscription?.plan?.price ?? 0} / {subscription?.plan?.duration_days ?? 0} days</p>
                        </div>
          ...
          {/* Support Card */}
          <Card className="bg-gradient-to-br from-purple-50 to-violet-50 border-2 border-purple-200 rounded-2xl shadow-xl mt-8">
            <CardContent className="p-6">
              <div className="flex items-center gap-3">
                <Headphones className="w-6 h-6 text-purple-600" />
                <div>
                  <h3 className="font-semibold text-gray-900">Need Help with Your Subscription?</h3>
